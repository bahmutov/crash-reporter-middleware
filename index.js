...
function initRaygunClient(config, addRenderValue, commitId) {
  ...
  if (check.unemptyString(raygunApiKey)) {
    raygunClient = new raygun.Client().init({ apiKey: raygunApiKey });
    ...
    addRenderValue('raygunExcludedHostnames', JSON.stringify(['localhost', '127.0.0.1']));
    console.log('initialized error reporting, commit id %s, version %s',
...
function initCrashReporting(config, server) {
  ...
  var hookReporter = initRaygunClient.bind(null, config, addRenderValue);
